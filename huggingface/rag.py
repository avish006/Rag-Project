...
def query_rag(user_query, index, chunks, image_metadata, chat_history, api_key):
    """Run hybrid retrieval then call Gemini to generate an answer."""
    try:
        # Instantiate the client per-request
...
def handle_query(user_query, api_key):
    ...
    answer = query_rag(user_query, index, chunks, image_metadata, chat_history, api_key)
