...
def process_query():
    try:
        ...
        if 'api_key' not in data or not data['api_key'].strip():
            return jsonify({"error": "Missing API Key. Please provide a Google Gemini API Key."}), 401
        
        result = handle_query(data['query'].strip(), data['api_key'].strip())
